SproutCore.registerLib("geometry", [], (game) => {
  game.geo = class Geo {
    /**
    ...
    /**
     * 
     * @param {Geo.shape} shape1 
     * @param {Geo.shape} shape2 
     * @param {function(Geo.shape, Geo.shape): boolean} test 
     */
    ...
    static intersect(shape1, shape2) {
      ...
      let test = Geo.intersectionTests[key1];
      if (test) {
        test = test[key2];
    ...
    static rec = class Rectangle extends Geo.shape {
      static name = "rec";
      constructor(x, y, w, h) {
        super();
        this.x = x;
    ...
    static circle = class Circle extends Geo.shape {
      static name = "circle";
      constructor(radius) {
        super();
        this.r = radius;
      ...
      convex(vertCount) {
        ...
        return new game.geo.convex(...verts);
      }
    ...
    static lineSeg = class LineSeg extends Geo.shape {
      static name = "line";
      ...
      intersectTU(other) {
        ...
        let rxs = game.geo.cross(adx, ady, bdx, bdy);
        if (rxs === 0) {
          // Colinear, may or may not intersect
          if (game.geo.cross(dox, doy, adx, ady) === 0) return [NaN, NaN];
          // Parallel, non-intersecting
          return [Infinity, Infinity];
        }
        // Lines intersect, bounds still need to be checked.
        return [
          game.geo.cross(dox, doy, bdx, bdy) / rxs,
          game.geo.cross(dox, doy, adx, ady) / rxs
        ];
      }
     ...
     */
    static convex = class Convex extends Geo.shape {
      static name = "convex";
      /**
  ...
  game.geo.shapeRegistry = {
    rec: game.geo.rec,
    circle: game.geo.circle,
    line: game.geo.lineSeg,
    lineseg: game.geo.lineSeg
  };

  // Registry of functions to transfer shape dimensions in a relatively sensible way
  // TODO: Unit test for sanity
  game.geo.conversions = {
    rec: {
      circle: (src) => { return new game.geo.circle(src.cx, src.cy, Math.min(src.w, src.h)/2); },
      line: (src) => { return new game.geo.lineSeg(src.x, src.y, src.r, src.d); },
      convex: (src) => { return new game.geo.convex(src.x, src.y, src.r, src.y, src.r, src.d, src.x, src.d); }
    },
    circle: {
      rec: (src) => { return new game.geo.rec(src.x-src.r, src.y-src.r, src.r*2, src.r*2); },
      line: (src) => { return new game.geo.lineSeg(src.x - src.r, src.y, src.x + src.r, src.y); },
      convex: (src) => { return new game.geo.convex(...src.convex(32)); }
    },
    line: {
      rec: (src) => { return new game.geo.rec(...src.bounds); },
      circle: (src) => { return new game.geo.circle(src.cx, src.cy, src.length/2); },
      convex: (src) => { return new game.geo.convex(src.ax, src.ay, src.bx, src.by); }
    },
    convex: {
      rec: (src) => { return new game.geo.rec(...src.bounds); },
      circle: (src) => { return new game.geo.circle(src.cx, src.cy, Math.min(src.w, src.h)/2); },
      line: (src) => { return new game.geo.lineSeg(...src.bounds); }
    }
  };

  // All intersections are exact

  // circle-circle intersection
  game.geo.setIntersectionTest(game.geo.circle, game.geo.circle, (a, b) => {
    return Math.hypot(b.y-a.y, b.x-a.x) <= a.r+b.r;
  });

  // line-line intersection
  game.geo.setIntersectionTest(game.geo.lineSeg, game.geo.lineSeg, (a, b) => {
  ...
  game.geo.setIntersectionTest(game.geo.rec, game.geo.rec, (a, b) => {
  ...
  game.geo.setIntersectionTest(game.geo.rec, game.geo.circle, (a, b) => {
    // Wide AABB
    if (game.geo.pointInAABB(b.x, b.y, a.x - b.r, a.y, a.w + b.r + b.r, a.h)) return true;
    // Tall AABB
    if (game.geo.pointInAABB(b.x, b.y, a.x, a.y - b.r, a.w, a.h + b.r + b.r)) return true;
    // Corner test
    if (b.x < a.x + a.w/2) {
      // Left side
      if (b.y < b.y + b.h/2) {
        // Top left
        return game.geo.pointInCircle(b.x, b.y, a.x, a.y, b.r);
      } else {
        // Bottom left
        return game.geo.pointInCircle(b.x, b.y, a.x, a.y, b.r);
      }
      ...
      if (b.y < b.y + b.h/2) {
        // Top right
        return game.geo.pointInCircle(b.x, b.y, )
      }
    }
  });

  // aabb-line intersection
  game.geo.setIntersectionTest(game.geo.rec, game.geo.lineSeg, (a, b) => {
    // If one or both line endpoints are in the rectangle, intersection is true.
    if (game.geo.pointInAABB(b.ax, b.ay, a.x, a.y, a.w, a.h)) return true;
    if (game.geo.pointInAABB(b.bx, b.by, a.x, a.y, a.w, a.h)) return true;
    // Otherwise, the line MUST be intersecting one of the sides for intersection to be true.
    // Top
    if (game.geo.lineSeg.intersect(b.ax, b.ay, b.bx, b.by, a.x, a.y, a.r, a.y)) return true;
    // Right
    if (game.geo.lineSeg.intersect(b.ax, b.ay, b.bx, b.by, a.r, a.y, a.r, a.d)) return true;
    // Bottom
    if (game.geo.lineSeg.intersect(b.ax, b.ay, b.bx, b.by, a.r, a.d, a.x, a.d)) return true;
    // Left
    if (game.geo.lineSeg.intersect(b.ax, b.ay, b.bx, b.by, a.x, a.d, a.x, a.y)) return true;
    return false;
  });

  // line-circle intersection
  game.geo.setIntersectionTest(game.geo.lineSeg, game.geo.circle, (a, b) => {
    let dotProd = a.delerp(b.x, b.y);
    if (dotProd < 0) return game.geo.pointInCircle(a.ax, a.ay, b.x, b.y, b.r);
    if (dotProd > 1) return game.geo.pointInCircle(a.bx, a.by, b.x, b.y, b.r);
